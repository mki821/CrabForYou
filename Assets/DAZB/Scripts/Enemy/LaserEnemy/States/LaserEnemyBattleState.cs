...
public class LaserEnemyBattleState : EnemyState<LaserEnemyStateEnum> {
    private readonly int battleModeHash = Animator.StringToHash("BattleMode");

    private LaserEnemy enemy;
    private Animator anim;

    public LaserEnemyBattleState(Enemy enemy, EnemyStateMachine<LaserEnemyStateEnum> stateMachine, string animBoolName) : base(enemy, stateMachine, animBoolName)
    {
    ...
    public override void UpdateState() {
        base.UpdateState();
        if (enemy.isDead) stateMachine.ChangeState(LaserEnemyStateEnum.Dead);
        ...
        if (enemy.IsPlayerInRange(enemy.canAttackCheckOffset, enemy.canAttackRange)) {
            ...
            if (enemy.CanAttack()) {
                stateMachine.ChangeState(LaserEnemyStateEnum.Attack);
            }
        }
        else {
